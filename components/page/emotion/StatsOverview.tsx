...
export const StatsOverview: React.FC<StatsOverviewProps> = ({
  ...
  return (
    <View className="gap-4">
      {/* Overview Card */}
      <View
        className={`rounded-xl p-4 shadow-sm ${isDark ? "bg-[#1a1a1a]" : "bg-white"}`}
      >
        <Text
          ...
          {topEmotion && (
            <View className="items-center">
              <View
                ...
                style={{ backgroundColor: topEmotion.color + "20" }}
              >
                <LottieView
                  source={getEmotionLottie(topEmotion.emotion)}
                  autoPlay
                  loop
                  speed={0.9}
      ...
      >
        ...
        {chartData.map((item) => (
          <View key={item.emotion} className="flex-row items-center mb-3">
            ...
            <View
              className={`flex-1 h-6 rounded-full mx-2 overflow-hidden ${isDark ? "bg-gray-800" : "bg-gray-100"}`}
            >
              <View
            ...
            <Text
              className={`text-sm font-semibold w-10 text-right ${isDark ? "text-white" : "text-gray-900"}`}
            >
              {item.count}
            </Text>
          </View>
