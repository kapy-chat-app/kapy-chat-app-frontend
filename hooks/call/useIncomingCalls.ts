...
export const useIncomingCalls = () => {
  ...
  const handleIncomingCallFromNotification = useCallback(
    ...
    const appStateSubscription = AppState.addEventListener(
      ...
      async (nextAppState: AppStateStatus) => {
        if (nextAppState === "active") {
          console.log(
            "📱 App became active, checking for pending call notifications..."
          );

          const notifications =
            await Notifications.getPresentedNotificationsAsync();

          const callNotification = notifications.find((notif) => {
            const data = notif.request.content.data as CallNotificationData;
            return data.type === "call" && data.action === "incoming_call";
          });

          if (callNotification) {
            const data = callNotification.request.content
              .data as CallNotificationData;
  ...
  useEffect(() => {
    ...
    newSocket.on("callEnded", async (data: any) => {
      ...
      if (data.call_id === currentCallIdRef.current) {
        ...
        setShowIncomingCall(false);
        currentCallIdRef.current = null;
        isHandlingNotification.current = false; // ⭐ Reset handling flag

        if (callTimeoutRef.current) {
          clearTimeout(callTimeoutRef.current);
          callTimeoutRef.current = null; // ⭐ Clear ref
        }
      }
    });

    newSocket.on(
      "callParticipantsUpdated",
      (data: { call_id: string; participants_count: number }) => {
